...
#    WM foi o melhor modelo testado´-fev 06 2024
#       ele foi feito como 3 labels para nao perder qualidade e ficar com numero 
#          adequado de parametros
#       este codigo imprime as regras
library(frbs)
library(readxl)
library("writexl")
...
method.type <- "WM"  # "WM","ANFIS","DENFIS","FS.HGD"
##    type.mf ="GAUSSIAN" "TRAPEZOID"
##    num.labels = 3, 15
control_WM <- list(num.labels = 5, max.iter=100, type.mf = "TRAPEZOID", type.defuz = "WAM",
                type.tnorm = "MIN", type.snorm = "MAX", type.implication.func = "ZADEH",
                name = "sim-0")

control_ANFIS <-list(num.labels=3, max.iter=50, step.size = 0.01,
                     type.tnorm = "MIN", type.snorm = "MAX",
                     type.implication.func = "ZADEH")
control_TSK = list(num.labels=5,  max.iter = 200, step.size = 0.01,
                   type.tnorm = "MIN", type.snorm = "MAX", 
                   type.implication.func = "ZADEH")
...
resp = c()
    ...
    frbs_model <- frbs.learn(data.train, range.data = range_matrix ,
                             method.type = method.type, control = control_WM)
